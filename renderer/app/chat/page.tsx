...
export default function ChatPage() {
  ...
  function openPendingItem(pending: BriefItem) {
    ...
    if (pending.path.startsWith("tasks/") && pending.id.startsWith("task-")) {
      ...
      activeTaskIdRef.current = pending.id;
      setShowRunCard(true);
      window.localStorage.setItem("firstmate:active-session", JSON.stringify({ path: `sessions/${pending.id}.md`, id: pending.id, title: pending.title, time: "", status: pending.status, agentId: "" }));
  ...
  async function openSession(session: RecentSession) {
    ...
    try {
      ...
      activeTaskIdRef.current = session.id;
      setShowRunCard(false);
      window.localStorage.setItem("firstmate:active-session", JSON.stringify({ ...session, status, agentId }));
      setBriefOpen(false);
  ...
  async function restoreActiveSession() {
    const rawSession = window.localStorage.getItem("firstmate:active-session");
    if (!rawSession) return;
    try {
      await openSession(JSON.parse(rawSession) as RecentSession);
    } catch {
      setRuntimeNotice("当前会话无法恢复。");
    }
  }
  function startNewConversation() {
    ...
    setShowRunCard(false);
    setDraft("");
    window.localStorage.removeItem("firstmate:active-session");
  ...
  useEffect(() => {
    void window.firstmate?.workspace.initialize().then(async () => {
      ...
      if (!window.localStorage.getItem("firstmate:open-session") && !window.localStorage.getItem("firstmate:new-conversation")) await restoreActiveSession();
    });
  ...
  async function send(event: FormEvent) {
    ...
    try {
      ...
      setShowRunCard(true);
      window.localStorage.setItem("firstmate:active-session", JSON.stringify({ path: created.sessionPath, id: created.id, title: content.slice(0, 80), time: "", status: created.status, agentId: created.agentId }));
      await refreshBrief();
  ...
  async function runWithModel(task = run) {
    ...
    try {
      ...
      if (result.status === "completed") {
        ...
        if (activeTaskIdRef.current === task.id) setRuntimeNotice(`模型结果已写入 ${result.outputPath}`);
        return;
      }
      if (activeTaskIdRef.current === task.id) setRuntimeNotice("模型凭证未配置，任务保持等待状态。");
    } catch (error) {
      const message = error instanceof Error ? error.message : "模型执行失败";
      setRun((current) => current?.id === task.id ? { ...current, status: "failed" } : current);
      await refreshBrief();
      notifyWorkspaceChanged();
      if (activeTaskIdRef.current === task.id) setRuntimeNotice(message.includes("insufficient_balance") || message.includes("402") ? "MiniMax 已收到请求，但账户余额不足（错误 1008）。充值或更换可用 API Key 后重试。" : message);
    }
  }
  ...
  return <>
    ...
    <div className="chat" ref={chatRef} onScroll={updateBottomState}><div className="date">{brief.generatedAt ? `${new Date(brief.generatedAt).toLocaleDateString()} / WORKSPACE BRIEF` : "WORKSPACE BRIEF"}</div>
